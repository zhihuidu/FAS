...
def compute_change(nodes, prev,cur,in_adj, out_adj,id_mapping, new_id_mapping):
...
def listshuffle(vertexlist,in_edges,out_edges,id_mapping,new_id_mapping):
    ...
    for perm in perms:
        ...
        delta = compute_change(vertexlist, current_order, perm, in_edges,out_edges,id_mapping, new_id_mapping)
        #for index,node in enumerate (perm) :
...
def exchange(pair,in_edges,out_edges,id_mapping,new_id_mapping):
    ...
    current_order = [id_mapping[idx] for idx in pair]
    best_order = current_order.copy()
    best_delta = 0    
    perm=[]
    perm.append(pair[1])
    perm.append(pair[0])
    delta = compute_change(pair,current_order,perm, in_edges,out_edges,id_mapping,new_id_mapping)

    if delta > 0:
        id_mapping[pair[0]]  = best_order[1] #new_id_mapping[pair[0]]
        id_mapping[pair[1]]  = best_order[0] #new_id_mapping[pair[1]]
    else:

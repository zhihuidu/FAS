...
#we can use this as the starting version to check the real graph connectome_graph.csv
#run it as this
#python feedforward.py connectome_graph.py 0
#you can change 0 to 1 or 2 if you want more detailed output information 
...
def compute_order(connections, verbosity=0, random_seed=0):
    ...
    print(f"Feedback size: {fsw}:{total_edge_weight} ({percentage_str(fsw, total_edge_weight)})")
    return (feedback_val,res)
...
#let the data as a graph
graph=data_table
OldVal=0;
oldval=999999999
itertimes=1
while True:
    print("Start the calculation ",itertimes)
    (curval,output) = compute_order(graph,int(verbosity),itertimes)
    if curval<oldval:
         print("New feedback value is", curval)
         print(output)
         print("New feedback value is", curval)
         oldval=curval
    else :
         print("No improvement value is", curval, " compared with ", oldval)

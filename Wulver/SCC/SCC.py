...
def process_graph(file_path):
    ...
    with open(oneoutput_file, 'a') as f:
            f.write(f"reading file {file_path} and find SCC takes {executiontime} seconds \n\n")
    removed_edges=[]
    removed_weight=0
    total_weight= total_edge_weight(G)
    removed_edge_file = f"{FileNameHead}-SCC-Removed-edge-{time_string}.csv"
    numcom=0
    #print(f"scc={scc}")
    for component in scc:
        ...
        if len(component)<2:
            continue
        print(f"handle the {numcom}th component")
        numcom += 1
        comG=build_graph_from_component(G,component)
        removed_edges1=solve_ip_scc(comG)
        removed_edges += removed_edges1
        removed_weight += removed_edge_weight(comG, removed_edges1)
        print(f"In this loop, number of removed edge is {len(removed_edges1)}")
        print(f"Totally removed weight is {removed_weight},percentage is {removed_weight/total_weight*100}\n\n")
        with open(removed_edge_file, 'a') as f:
            for u, v in removed_edges1:
        ...
        with open(oneoutput_file, 'a') as f:
            ...
            f.write(f"Percentage of Removed Edges Weight: {removed_weight/total_weight*100}\n\n")


    starttime = time.time()
    G_nocycle = construct_shrunk_graph(G, removed_edges)
    G_dag_relabelled, mapping = relabel_dag(G_nocycle)

...
num=0
def parallel_remove_cycle_edges(nodes,edge_weights,out_adj,edge_flag):
    oldnum=num
    removed_weight=0
...
def divide_set(input_set, part_size):
   parts = []
   component=input_set
   samplesize=max(part_size,len(component))
   while len(input_set) > 0:
          smallcom=random.sample(component, samplesize)
          parts.append(set(smallcom))
...
def process_graph(file_path):
    ...
    while not nx.is_directed_acyclic_graph(shG):
        ...
        for component in scc:
            ...
            subnum=0
            allsetlist=[]
            if len(component)>9000:
                  allsetslist = divide_set(component,9000)
                  with concurrent.futures.ThreadPoolExecutor() as executor:
                      for subset in allsetslist:
                          executor.submit(parallel_remove_cycle_edges,list(subset), edge_weights,out_edges,edge_flag )
            else:
                  sccdfs_remove_cycle_edges(list(component), edge_weights,out_edges,edge_flag)
            removednum=0
            removed_weight=0
            for u,v in edge_flag:
                if edge_flag[(u,v)]==0:
            ...
            print(f"to here removed {removednum} edges, removed weight is {removed_weight}, percentage is {removed_weight/total*100}")


        print(f"build dag")
        shG=build_dag(edge_weights,edge_flag)
        dag_weight=sum(data['weight'] for u, v, data in shG.edges(data=True))
        print(f"shunk graph weight={dag_weight},percentage={dag_weight/total*100}")

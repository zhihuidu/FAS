...
def partial_topological_sort_and_reverse_edges(graph,edge_flag):
    ...
    while len(graph) > 0:
        ...
        # Add all sources to the topological sort
        topo_sort.extend(sources)
        graph.remove_nodes_from(sources)

    return topo_sort, reverse_edges
    ...
    while add_pointer>0 :
        ...
        try:
                     ...
                     if removed_weight<add_weight:
                          ...
                          print(f"restore the old edge flag")
        except :

...
void improve_solution(Graph *graph, Ordering *ordering, int *current_score) {
	...
	selected_nodes[1]=v;
        ...
        int original_positions[2];
        for (int i = 0; i < 2; i++) {
            original_positions[i] = ordering->position[selected_nodes[i]];
	    //printf("original %d -> %d \n",selected_nodes[i],original_positions[i]);
        }
		...
		//printf("edge %d (%d,%d) old contribution is %d\n",edge_index,u,v,contributes_old );
		//printf("edge %d (%d,%d) new contribution is %d\n",edge_index,u,v,contributes_new );
            ...
            if (delta_score > 0) {
                ...
                *current_score += delta_score;
                ...
                char filename[50];
                sprintf(filename, "%d.csv", *current_score);

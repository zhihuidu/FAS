...
void generate_permutations(int *nodes, int start, int end, int permutations[][SS], int *count);
...
void improve_solution(Graph *graph, Ordering *ordering, int *current_score) {
    ...
    for (int paralli=0; paralli < 8; paralli++) {
        ...
        int selected_nodes[SS];
        int selected_count = 0;
        while (selected_count < SS) {
        ...
        int permutations[PN][SS];
        int perm_count = 0;
        int nodes_copy[SS];
        memcpy(nodes_copy, selected_nodes, SS * sizeof(int));
        generate_permutations(nodes_copy, 0, SS-1, permutations, &perm_count);
        ...
        int original_positions[SS];
        int new_positions[SS];
        for (int i = 0; i < SS; i++) {

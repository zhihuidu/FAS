...
def solve_indicator_half_linear(graph,edge_flag,initial=False,checkpoint_file=None):
    ...
    model.setParam('Heuristics', 0.45)  # 30% of the time spent on heuristics
    model.setParam('CutAggPasses', 3)  # More aggressive cutting
    model.Params.VarBranch = 1  # Alternative branching strategy
    model.Params.BranchDir = 1  # Explore down branches first
    model.setParam('Cuts', 0)          # Moderate cut generation, larger cuts will be slow
    model.setParam('BarConvTol', 1e-3)  # More aggressive convergence tolerance
    ...
    model.setParam('MIPGap', 0.15)      # Allow a 10% optimality gap

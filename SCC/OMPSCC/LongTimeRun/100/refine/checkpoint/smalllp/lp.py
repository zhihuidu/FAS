...
'''
    ...
    if model.status == GRB.OPTIMAL or model.status == GRB.SUBOPTIMAL:
        ...
        output_file = f"{FileNameHead}-solution-edge-{time_string}.csv"
        with open(output_file, 'w') as f:
               for u,v in graph.edges():
                    f.write(f"{u},{v},{graph[u][v]['weight']},{x[(u,v)].X}\n")
        output_file = f"{FileNameHead}-solution-vertex-{time_string}.csv"
        with open(output_file, 'w') as f:

...
def solve_fas_with_weighted_ip(graph,edge_flag,initial=False,checkpoint_file=None):
    ...
    model.setParam('TimeLimit', 10)    # Set a time limit of 30 seconds
    '''
...
'''
    ...
    for u, v in graph.edges():
        x[(u, v)] = model.addVar(vtype=GRB.BINARY, name=f"x{u}-{v}")
    ...
    for v in graph.nodes():
        p[v] = model.addVar(vtype=GRB.CONTINUOUS,lb=0,ub=M-1, name=f"p{v}")
    ...
    for u, v in graph.edges():
        ...
        model.addConstr(p[u]+ 1 <= p[v] + M * (1 - x[(u, v)]), f"order{u}-{v}")
    ...
    if checkpoint_file:
        ...
        model.update()
        if os.path.exists("ipcheckpoint.sol"): 
        ...
        # If the optimization found a feasible or optimal solution
        if model.status in [GRB.TIME_LIMIT, GRB.INTERRUPTED]:
            # Save the model
            print(f"write model")
            model.write("ipcheckpoint.mps")
    
            # Save the solution
            if model.status != GRB.INFEASIBLE:
                print(f"write solution")
            ...
            if len(component)<10:
                ...
                try:
                     if checkpointfile == None:
                         ...
                         removed_weight1=solve_fas_with_weighted_ip(G_sub,edge_flag,False,"ipcheckpoint.mps")

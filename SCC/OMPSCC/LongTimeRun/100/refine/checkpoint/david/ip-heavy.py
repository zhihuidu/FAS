...
def calculate_heavy_set(G,percentage):
    ...
    print(f"add heavy in degree vertices {len(heavyset)} with {percentage} ")
    select_heavy_node(out_degrees, out_degree_stats, percentage, heavyset)
    print(f"add heavy in+ out degree vertices {len(heavyset)} with {percentage} ")
...
def top_k(G,start,K):
    ...
    top_k_heavy_vertices = [node for node, degree in sorted_vertices[start:K+start]]
    return set(top_k_heavy_vertices), sorted_vertices[K-1][1]
...
def solve_fas_with_weighted_ip(graph,edge_flag):
    ...
    print(f"removed {len(removededge)} edges, weight is {removed_weight}")
...
def process_graph(file_path,precondition=0,checkpoint_file=None):
    ...
    while not acyclic_flag :
        ...
        for component in scc:
            ...
            subnum=0
            G_com = shG.subgraph(component).copy()
            if len(component)<1000:
                     removed_weight1=solve_fas_with_weighted_ip(G_com,edge_flag)
            ...
            setsize=50
            start=0
            while len(component) >=1000:

                   #smallcom=random.sample(component, subcomponentsize)
                   setsize=int(setsize*math.sqrt(numcheckacyclic))
                   if setsize > len(component)  :
                       setsize= len(component)
                   heavyset,degreesum =top_k(G_com,start,setsize)
                   start+=setsize
                   ...
                   if degreesum < 50:
                       ...
                       setsize+=1
                   print(f"select {len(heavyset)} vertices, degree sum is larger than {degreesum}")

                   G_sub = G_com.subgraph(heavyset).copy()
                   removed_weight1=solve_fas_with_weighted_ip(G_sub,edge_flag)
                   removed_weight+=removed_weight1
                   print(f"size of component is {len(component)}")
                   component.difference_update(heavyset)
                   print(f"after remove {len(heavyset)} vertices, size of component is {len(component)}")
                   subnum += 1
                   print(f"{numcheckacyclic} check, handle the {subnum}th random part of {numcomponent}th component with size {len(component)}")
                   print(f"subset removed weight is {removed_weight1}, totally removed {removed_weight}, percentage is {removed_weight/total*100}\n")

    ...
    while not acyclic_flag :
        ...
        for component in scc:
            ...
            while len(component) >=1000:
                   ...
                   setsize=int(500*math.sqrt(numcheckacyclic))
                   if setsize > len(component):
                       setsize=len(component)
                   smallcom=random.sample(sorted(component), setsize)
                   component.difference_update(smallcom)

                   G_sub = G_com.subgraph(smallcom).copy()

                   removed_weight1=solve_fas_with_weighted_ip(G_sub,edge_flag)
                   removed_weight+=removed_weight1

                   subnum += 1
                   print(f"{numcheckacyclic} check, handle the {subnum}th random {setsize} vertices of {numcomponent}th component with size {len(component)}")
                   print(f"subgraph removed weight is {removed_weight1}, totally removed {removed_weight}, percentage is {removed_weight/total*100}\n")

            if  len(component)>1:

                   G_sub = G_com.subgraph(component).copy()
                   removed_weight1=solve_fas_with_weighted_ip(G_sub,edge_flag)
                   removed_weight+=removed_weight1
                   subnum += 1

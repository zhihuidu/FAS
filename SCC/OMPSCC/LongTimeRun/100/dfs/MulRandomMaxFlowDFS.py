...
FileNameHead="Random-SCC-OMP-DFS"
...
def process_graph(file_path):
    ...
    while not nx.is_directed_acyclic_graph(shG):
        ...
        for component in scc:
            ...
            totalsum2=0
            totalsum0=0
            if  len(component) >=1000 and numcheckacyclic % 10 ==0 :
                        G_sub = shG.subgraph(component).copy()
                        ...
                        print(f"Randomly select vertex set with {numpair} pairs of vertices for maxflow")
                        sourcelist=[]
                        targetlist=[]
                        for i in range(numpair):
                            target=random.choice(list(component))
                            source=random.choice(list(component))
                            if source!=target and nx.has_path(G_sub,source,target) and nx.has_path(G_sub,target,source):
                               G_sub.add_edge(0,source,weight=99999999)
                               G_sub.add_edge(target,1,weight=99999999)
            ...
            if  1==1:
                   removed_weight1= ompdfs_remove_cycle_edges(component,shG, fixcyclelen,mincyclelen,numcycles,maxcyclelen,time_limit,edge_flag)
                   removed_weight+=removed_weight1
                   print(f"The {numcomponent}th component, removed weight is {removed_weight1}, totally removed {removed_weight}, percentage is {removed_weight/total*100}\n")
                   subnum += 1
                   print(f"{numcheckacyclic} check, handle the {subnum}th random part of {numcomponent}th component with size {len(component)}")

...
# call C Edmond-Karp algorithm to break the cycles between source and target
def EdmondsKarp(G,source,target,edge_flag):
    removed_weight=0
    global num

    commandline=f"rm -f tmp-maxflow-file.csv tmp-maxflow-removed-edges.csv"
    ...
    with open("tmp-maxflow-file.csv", 'w') as f:
        for u, v, data in G.edges(data=True):
            if edge_flag[(u,v)]==1:
               f.write(f"{node_to_idx[u]},{node_to_idx[v]},{data['weight']}\n")
    # search cycles in the subgraph
    commandline=f"./maxflow  tmp-maxflow-file.csv {len(nodes)} 0 {node_to_idx[source]} {node_to_idx[target]}"
    if os.system(commandline)!=0 :
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        print("There is an error during call maxflow")
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n")

    if os.path.exists("tmp-maxflow-removed-edges.csv"):
        with open("tmp-maxflow-removed-edges.csv",mode='r') as f:
            csv_reader = csv.reader(f)
            firstline=next(csv_reader)
            removed_weight=int(firstline[0])
            for row in csv_reader:
                source = idx_to_node[int(row[0])]
                dest = idx_to_node[int(row[1])]
                weight = int(row[2])  
                if edge_flag[(source,dest)]==1:
                    edge_flag[(source,dest)]=0
                    num+=1
            ...
            if  len(component) >1000:
                        ...
                        for i in range(numpair):
                            ...
                            if source!=target and nx.has_path(G_sub,source,target) and nx.has_path(G_sub,target,source):
                                      ...
                                      weight1=EdmondsKarp(G_sub,source,target,edge_flag)
                                      sum2+=weight1
                        removed_weight+=sum2

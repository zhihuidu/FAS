...
def find_cut_edges(graph, node_to_label):
    cut_edges = []
...
'''
    ...
    while not nx.is_directed_acyclic_graph(shG):
        ...
        for component in scc:
            ...
            if len(component)>=40000:
                  G_sub = G.subgraph(component).copy()
                  node_to_label = spectral_clustering_divide(G_sub)
                  sourceset=set()
                  targetset=set()
                  for i in node_to_label:
                      if node_to_label[i]==0:
                          sourceset.add(i)
                      else:
                          targetset.add(i)
                  sourcelist=list(sourceset)
                  targetlist=list(targetset)
                  for i in range(min(len(sourcelist),len(targetlist))):
                      source=sourcelist[i]
                      target=targetlist[i]
